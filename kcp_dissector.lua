...
--]]
    ...
    KCP.fields = {conv, cmd, frg, wnd, ts, sn, una, len, data}
    ...
    -- KCP dissect packet
    function KCP.dissector(Buffer, pinfo, T)
        ...
        while offset < Buffer:len() do
            ...
            local length = LEN:le_uint()
            Tree:append_text(", data: " .. length)
            if length > 0 then
            ...
            if CMD:le_uint() == 81 then
                local temp = "CMD_PUSH, SN(" .. SN:le_uint() .. ")"
                if UNA:le_uint() > 0 then
                    temp = temp .. ", WAIT_SN(" .. UNA:le_uint() .. ")"
                end
                info = info .. "[" .. temp .. "]"
            elseif CMD:le_uint() == 82 then
                local temp = "CMD_ACK,  SN(" .. SN:le_uint() .. "), NEXT_SN(" .. UNA:le_uint() .. ")"
                info = info .. "[" .. temp .. "]"
            elseif CMD:le_uint() == 83 then
                -- TODO
            elseif CMD:le_uint() == 84 then
